...
long long countArrangements(std::string configuration, std::vector<int> groups) {
    if (CACHE.find(std::make_pair(configuration, groups)) != CACHE.end()) return CACHE[std::make_pair(configuration, groups)];
    if (groups.size() == 0) {
    ...
    CACHE[std::make_pair(configuration, groups)] = ret;
    return ret;
}
...
std::string solve(char** argv) {
    ...
    for (int i = 0; i < configurations.size(); i++) {
        std::string repeatedConfiguration = "";
        std::vector<int> repeatedGroups;
        for (int rep = 0; rep < 5; rep++) {
            if (rep < 4) repeatedConfiguration += configurations[i] + "?";
            else repeatedConfiguration += configurations[i];
        
            for (int n : groups[i]) repeatedGroups.push_back(n);
        }

        ans += countArrangements("." + repeatedConfiguration + ".", repeatedGroups);
    }
